...
factorial = {}
for i in range(0, 50):
    factorial[i] = np.longdouble(np.math.factorial(i))
...
def error_sum(h, s, m, cs, cs_y, maxc = 1, maxp = 30, use_max = True, n = None):
    r"""
    h: step size
    n: number of spins
    ...
    acc_bound_taylor_omega = accumulate_from(2*s+1, bound_taylor, maxp)
    ...
    psi_m_taylor_order_error = Psi_m_Taylor_error(h, maxp, s, m, cs[s][m], use_max = use_max or s > 4)
...
hs = [1/2**(i/4+3) for i in range(1,500)]
total_error_list = [1e-19]
total_time_list = [2**i for i in range(3, 11)]
range_s = [1,2,3,4]
range_m = [1,2,5,11]
...
def compute_step_error(hs, range_s, range_m, maxp, total_error_list, total_time_list, use_max = True):
    ...
    for total_error in tqdm(total_error_list, desc='total_error'):
        ...
        for n in tqdm(total_time_list, desc = 'time'):  
            ...
            for (s, m) in zip(range_s, range_m):
...
step_error = compute_step_error(hs, range_s = [1,2,3,4,5], range_m = [1,2,3,4,5], maxp = 30, total_error_list = total_error_list, total_time_list = total_time_list, use_max = True)
...
for total_error, ax in zip(total_error_list, ax.flatten()):
    for (s, m, c) in zip(range_m, range_s, colors):
        min_costs = []
        min_costs_h = []
